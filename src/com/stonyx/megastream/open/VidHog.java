...
public class VidHog extends Source
{
  ...
  private class OpenTask extends Source.OpenTask
  {
    ...
    protected Boolean doInBackground(URL... params)
    {
      ...
      try
      {
        wait = Integer.parseInt(GetGroup("<span id=\"countdown_str\">Wait <span id=\".+?\">" + 
               "([0-9]+?)</span> seconds</span>", string));
      }
      ...
      Boolean waitResult = Wait(wait,
                           GetOpenMode() == OpenActivity.Mode.STREAM_OPEN ? 200 : 100,
                           GetOpenMode() == OpenActivity.Mode.STREAM_OPEN ? 900 : 450,
                           error);
      if (waitResult == null)
      {
        return true;
      ...
      post = CreatePostRequest(url,
             new String[] { "op", op, "id", id, "rand", rand, "method_free", "Free Download",
             "down_direct", "1" });
      if (post == null)
      {
        ShowErrorDialog(Strings.ErrorParseFailure);
      ...
      // Create the link
      String link = GetGroup("<a href=\"(http.+?)\">Click Here to download this file</a>",
                    page.toString());
  ...
  // Constructor
  public VidHog(OpenFragment fragment, TextView topText, ProgressBar progressBar,
