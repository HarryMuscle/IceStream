...
public class UploadOrb extends Source
{
  ...
  private class OpenTask extends Source.OpenTask
  {
    ...
    protected Boolean doInBackground(URL... params)
    {
      ...
      HttpPost post = CreatePostRequest(url,
                      new String[] { "op", op, "id", id, "fname", fname, "method_free", "Free" });
      if (post == null)
      {
        ShowErrorDialog(Strings.ErrorParseFailure);
        return false;
      ...
      // Check for a password protected link
      if (string.contains("input type=\"password\" name=\"password\""))
      {
        ShowErrorDialog(Strings.ErrorPasswordProtected);
        return false;
      }
      
      // Find the post data
      op = GetGroup("<input type=\"hidden\" name=\"op\" value=\"(.+?)\">", string);
      id = GetGroup("<input type=\"hidden\" name=\"id\" value=\"(.+?)\">", string);
      String rand = GetGroup("<input type=\"hidden\" name=\"rand\" value=\"(.+?)\">", string);
      if (op == null || id == null || rand == null)
      {
        ShowErrorDialog(Strings.ErrorParseFailure);
        return false;
      }
      ...
      // Send the post data
      post = CreatePostRequest(url,
             new String[] { "op", op, "id", id, "rand", rand, "method_free", "Free",
             "down_direct", "1" });
      if (post == null)
      {
        ShowErrorDialog(Strings.ErrorParseFailure);
      ...
      // Create the link
      String link = GetGroup("<FORM METHOD=\"LINK\" ACTION=\"(.+?)\">", page.toString());
  ...
  // Constructor
  public UploadOrb(OpenFragment fragment, TextView topText, ProgressBar progressBar,
