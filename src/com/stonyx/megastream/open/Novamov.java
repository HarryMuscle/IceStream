...
public class Novamov extends Source
{
  ...
  private class OpenTask extends Source.OpenTask
  {
    ...
    protected Boolean doInBackground(URL... params)
    {
      ...
      if (string.contains("file no longer exists"))
      {
      ...
      String file = GetGroup("flashvars.file=\"(.+?)\"", string);
      String key = GetGroup("flashvars.filekey=\"(.+?)\"", string);
      ...
      // Create the URL for communicating with the API and getting the actual URL
      String url = "http://www.novamov.com/api/player.api.php?" + 
                   URLEncodeData(new String[] { "file", file, "key", key });
      
      // Download the URL information
      ...
      // Create the link
      String link = GetGroup("url=(.+?)&title", page.toString());
  ...
  // Constructor
  public Novamov(OpenFragment fragment, TextView topText, ProgressBar progressBar,
