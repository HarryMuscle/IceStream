...
public class TwoShared extends Source
{
  ...
  private class OpenTask extends Source.OpenTask
  {
    ...
    protected Boolean doInBackground(URL... params)
    {
      ...
      // Conver the builder to a string
      String string = page.toString();

      // Check if the link is permanently unavailable
      if (string.contains("The file link that you requested is not valid"))
      {
      ...
      // Create the link
      String link = GetGroup("['\">](http://.+?2shared.com/download/.+?)['\"<]", string);
      ...
      if (GetOpenMode() == OpenActivity.Mode.DOWNLOAD_OPEN ||
          GetOpenMode() == OpenActivity.Mode.DOWNLOAD_SAVE)
      {
        get = CreateGetRequest(link, false);
        StringBuilder redirect = new StringBuilder();
        result = DownloadFile(client, get, saveLocation, 500, 1000, redirect, error);
        if (result == null)
        {
          return true;
        }
  ...
  // Constructor
  public TwoShared(OpenFragment fragment, TextView topText, ProgressBar progressBar,
